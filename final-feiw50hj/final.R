...
# y <- <your code here>
y=list(seq(1,1000,1),seq(2,1000,2),seq(3,1000,3),seq(4,1000,4),seq(5,1000,5),seq(6,1000,6),seq(7,1000,7),seq(8,1000,8),seq(9,1000,9),seq(10,1000,10))
...
set.seed(344)
#m <- <your code here>
m=matrix(rnorm(100,mean=8),10,10)
...
# 
# t2 <- <your code here>
infants2=infants[infants$ed=="College",]
t2=table(infants2$smoke,infants2$number)
...
bdiff=mean(infants$age[infants$age>=35],na.rm=T)-mean(infants$age[infants$age<35],na.rm=T)
#################################################################
...
coef=lm(mtcars$qsec~mtcars$hp)$coef
...
plot(mtcars$wt,mtcars$disp,col=as.numeric(mtcars$am)+1,xlab="weight of the car",ylab="engine displacement")
...
plot(WorldBank$fertility.rate,WorldBank$life.expectancy,pch=".",col=as.numeric(WorldBank$region),
     xlab="the fertility rate of each country", ylab="the life expectancy of each country")
abline(v=2.1)
...
# The left plot should include only data from 1960, the right one only from 2014. ###2013
par(mfrow=c(1,2))
WB1960=WorldBank[WorldBank$year==1960,]
WB2013=WorldBank[WorldBank$year==2013,]
plot(WB1960$fertility.rate,WB1960$life.expectancy,pch=".",col=as.numeric(WB1960$region),
     xlab="the fertility rate of each country", ylab="the life expectancy of each country")
abline(v=2.1)

plot(WB2013$fertility.rate,WB2013$life.expectancy,pch=".",col=as.numeric(WB2013$region),
     xlab="the fertility rate of each country", ylab="the life expectancy of each country")
abline(v=2.1)
# [4 pts]
...
IDA=WorldBank[WorldBank$lending=="IDA",]
hist(IDA$GDP.per.capita.Current.USD,breaks=seq(0,7000,250),xlab="GDP of countries whose lending rate is IDA",main="Histogram of GDP of Countries whose lending rate is IDA")
#################################################################
...
par(mfrow=c(2,3))
lapply(rain,FUN=function(x) hist(x))
#################################################################
...
PermDiff <- function(cases, controls, k=5000){
  diff=c()
  whole=c(cases,controls)
  m=length(controls) 
  for (i in 1:k){
    cases=sample(whole,k)
    casem=mean(cases)
    controlm=(sum(whole)-sum(cases))/m
    diff[i]=casem-controlm
  }
  return(diff)}
#################################################################
...
Onetime=function(k){
  winning=sort(sample(c(1:19),3))
  count=0
  for (i in 1:k) {
    player=sort(sample(c(1:19),3))
    if (all(player==winning))   count=count+1
  }
  return(count)  
}

NumJackpot=function(k, B){
  return(replicate(B,Onetime(k))) 
}

# For B = 5000/1000 and each value of k = 10000, 50000, 100000, 500000  /100
...
for (i in 1:length(phrases)){
  phrases2[i]=replaceitem(phrases[i])
}
