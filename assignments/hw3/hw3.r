...
# load the data
load("C:/Users/Zirui/src/stat133/assignments/hw3/WR1500MeterMen.rda")
...
#or wr.name=wr1500m$athlete[wr1500m$times==min(wr1500m$times)]
#or wr.name=wr1500m$athlete[wr1500m$times %in% min(wr1500m$times)]
...
ggplot(wr1500m,aes(wr1500m$year,wr1500m$times_sec))+geom_step()
...
new_year[is.na(wr1500m$month)]=wr1500m[is.na(wr1500m$month),"year"]+0.5
new_year[is.na(wr1500m$month)==FALSE]=wr1500m$year[is.na(wr1500m$month)==FALSE]+wr1500m$month[is.na(wr1500m$month)==FALSE]/12
# wr1500m <- your code here
...
# Your ggplot command:
ggplot(wr1500m,aes(year,times_sec))+geom_step()+xlim(1890,2020)+geom_segment(aes(x=1998,y=wr_1998,xend=2015,yend=wr_1998))
...
# Your ggplot command
ggplot(wr1500m,aes(year,times_sec))+geom_step()+xlim(1890,2020)+
  geom_segment(aes(x=1998,y=wr_1998,xend=2015,yend=wr_1998))+
    geom_vline(xintercept=1944,colour="green")+
       geom_vline(xintercept=1998,colour="green")+         
         annotate("text",x=1998+12,y=wr_1998+2,label=wr1500m$athlete[wr1500m$year==1998],color="blue")+
           annotate("text",x=1944+9,y=wr_1944+2,label=wr1500m$athlete[wr1500m$year==1944],color="blue")
...
ggplot(wr1500m,aes(year,times_sec))+geom_step()+xlim(1890,2020)+
  ...
  annotate("text",x=1998+12,y=wr_1998+2,label=wr1500m$athlete[wr1500m$year==1998],color="blue")+
  annotate("text",x=1944+9,y=wr_1944+2,label=wr1500m$athlete[wr1500m$year==1944],color="blue")+
  labs(x="Year",y="world record",title="Men 1500m World Record")
...
# Your ggplot command
ggplot(SO2012Ctry,aes(pop,GDP_per_person))+(size=SO2012Ctry$Total,shape=1)+scale_x_log10()+scale_y_log10()
...
# Your ggplot command:
ggplot(SO2012Ctry,aes(pop,GDP_per_person))+geom_point(size=SO2012Ctry$Total,shape=1)+
  scale_x_log10()+scale_y_log10()+labs(x="Population",y="GDP per person",title="GDP per person vs. Population")+
     annotate("text",x=SO2012Ctry$pop[top5],y=SO2012Ctry$GDP_per_person[top5],label=SO2012Ctry$ISO[top5])  
#attention: the function scale_x_log10() is appied to all the functions above, so all the x,y cordinate should not be set log10() 
#or: 
#ggplot(SO2012Ctry,aes(pop,GDP_per_person))+geom_point(size=SO2012Ctry$Total,shape=1)+
#  scale_x_log10()+scale_y_log10()+labs(x="Population",y="GDP per person",title="GDP per person vs. Population")+  
#    geom_text(data=SO2012Ctry,aes(pop[top5],GDP_per_person[top5],label=ISO[top5],group=1))  
#why the last command doesn't work?
...
ggplot(world,aes(long, lat,group=group)) + geom_polygon(fill="grey",color="light grey")
...
ggplot(world,aes(long, lat,group=group)) + geom_polygon(fill="grey40",color="grey90")+
  geom_point(data=wonMedal,aes(longitude,latitude,size=Total,group=1,color="gold"))
...
load("C:/Users/Zirui/src/stat133/assignments/hw3/London2012ALL_ATHLETES.rda")
...
# Your ggplot commands
ggplot(athletes,aes(Sport,fill=Sex))+geom_bar(position="dodge")+
  labs(title="Omlypic Games Gender Distribution of Different Activities")+
...
load("C:/Users/Zirui/src/stat133/assignments/hw3/rainfallCO.Rda")
...
mean.rain=sapply(rain,sd)

# Create a variable 
# n1989.rain : a vector of length 5 with the number of measurements at each station in the year 1989 (use [day])
l1 = length(rain$st050183[day$st050183 >= 1989 & day$st050183 < 1990])
l2 = length(rain$st050263[day$st050263 >= 1989 & day$st050263 < 1990])
l3 = length(rain$st050712[day$st050712 >= 1989 & day$st050712 < 1990])
l4 = length(rain$st050843[day$st050843 >= 1989 & day$st050843 < 1990])
l5 = length(rain$st050945[day$st050945 >= 1989 & day$st050945 < 1990])
n1989.rain = c(l1, l2, l3, l4, l5)
